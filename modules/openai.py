...
def voicetext(client, message: Message):
    ...
    try:
        ...
        message.edit('❌ Отклонено: отсутствует голосовое сообщение')
    try:
        ...
        message.edit('❌ Не удалось перевести голос, попробуйте ещё раз')
    media_file.close()
    os.remove(voice)
...
def gpt(client: Client, message: Message):
    ...
    if message.from_user.id == client.get_me().id:
        if message.text.split(' ',1)[1] == '+allgpt':
            allgpt = 1
            message.edit('✅ Режим "allgpt" включен\nВыключить: `!gpt -allgpt`',)
        elif message.text.split(' ',1)[1] == '-allgpt':
            allgpt = 0
            message.edit('✅ Режим "allgpt" выключен\nВключить: `!gpt +allgpt`')
        else:
            try:
                message.edit('🤖 ChatGPT 3.6 Turbo генерирует ответ...')
        ...
        if allgpt == 1:
            msgpt = client.send_message(message.chat.id, '🤖 ChatGPT 3.5 Turbo генерирует ответ...', reply_to_message_id=message.id)
            engine="gpt-3.5-turbo"
            prompt = message.text.split(' ',1)[1]
            completion = openai.ChatCompletion.create(model=engine, messages=[{"role": "user", "content": prompt}], temperature=0.9, max_tokens=3000)
            reply = completion.choices[0]["message"]["content"].replace("'", "```")
            reply = f'🤖: {reply}\n\n[🚹 {message.from_user.first_name}](tg://user?id={message.from_user.id})❓: {prompt}'
            replymore4k =  [reply[i:i+3000] for i in range(0, len(reply), 3000)]
...
def draw(client: Client, message: Message):
    ...
    if message.from_user.id == client.get_me().id:
        if message.text.split(' ',1)[1] == '+alldraw':
            alldraw = 1
            message.edit('✅ Режим "alldraw" включен\nВыключить: `!draw -alldraw`')
        elif message.text.split(' ',1)[1] == '-alldraw':
            alldraw = 0
            message.edit('✅ Режим "alldraw" выключен\nВключить: `!draw +alldraw`')
        else:
            try:
                prompt = message.text.split(' ',1)[1]
                message.edit('🖌️ DALL-E генерирует картинку...')
        ...
        if alldraw == 1:
            try:
                prompt = message.text.split(' ',1)[1]
                msgpt = client.send_message(message.chat.id, '🖌️ DALL-E генерирует картинку...', reply_to_message_id=message.id)
                response = openai.Image.create(prompt=prompt, n=1, size='1024x1024')
                image_url = response["data"][0]["url"]
                file_path = './bin/Mod/cache/photo_draw.jpg'
                download_image(image_url, file_path)
...
modules_help[module_name] = {
    ...
    "gpt":"Use ChatGPT online and for free\n+allgpt / -allgpt"
}